## 2026-08-06 — «Самый удобный и самый быстрый»: умный помощник, живой пайплайн и холодные оптимизации

Раунд полировки продукта: уникальные функции, скорость и «система, которая постоянно работает».

### Новые функции
- **Умный помощник редактора (src/components/editor/EditorAssistant.tsx).** Редактор
  анализирует состояние проекта и подсказывает следующий полезный шаг с одной кнопкой:
  «добавьте материалы», «соберите первый клип», «добавьте музыку», «проверьте монтаж
  (Picture Lock)», «завершите Picture Lock», «добавьте титр», «экспортируйте». Каждая
  подсказка выполняет реальное действие в сторе, скрывается персонально и не повторяется.
  Аналогов у Runway/Pika/Descript/OpusClip нет: те либо генерируют, либо требуют искать
  инструменты в меню.
- **Живой пайплайн генерации.** На главной вместо одной подписи — этапы
  «Материалы → AI-анализ и монтаж → Picture Lock → Рендер → Готово» с состояниями
  (сделано/активно/ожидает) и прогресс-баром. Пользователь всегда видит, что делает система.
- **Контекстная подсказка на главной.** Показывает, как система прочитает запрос
  (формат 9:16/16:9, темп, цветокоррекция, шаблон) — те же правила, что реально использует
  автомонтаж.
- **Карточки проектов с превью-кадрами** и кнопкой быстрого перехода в AI Director.

### Скорость (Part 2)
- **Таймлайн больше не перерисовывается 60 раз/с при воспроизведении.** Плейхед вынесен в
  лёгкий отдельный слой (PlayheadLayer), клипы обёрнуты в React.memo, автопрокрутка читает
  стор напрямую. Раньше каждый кадр воспроизведения перерисовывал все дорожки и «киноплёнку».
- **Панель транспорта:** таймкод вынесен в отдельный компонент — цифры обновляются 60 раз/с,
  кнопки и слайдеры не перерисовываются.
- **Предпросмотр:** когда воспроизведение остановлено и ничего не менялось, канвас не
  перерисовывает кадр 60 раз/с (экономия CPU/батареи; кисть VFX рисует вживую).
- **IndexedDB:** список проектов читается через индекс `updatedAt` вместо чтения всего и
  JS-сортировки.
- **Медиатека:** карта «ассет → число использований» строится одним проходом вместо
  O(ассетов × клипов).
- **Превью-кадры легче в ~3 раза** (240px/q0.66 вместо 320px/q0.75) — сохранения проекта и
  undo/redo сериализуют меньше данных.
- **Пустой проект открывается на «Медиа»** (импортировать материалы), собранный — на «Монтаж».

### «Пользователь не ждёт» (Part 3)
- **Фоновая разминка** (src/lib/editor/idleWarmup.ts): главная и редактор в простое
  подгружают тяжёлые модули (автомонтаж, рендер, Picture Lock, мастеринг), а затем — былм
  FFmpeg (пропуск на устройствах < 4 ГБ памяти). Экспорт и генерация стартуют мгновенно.
- **FFmpeg грузится параллельно с AI-анализом**: пока автомонтаж анализирует материалы,
  wasm движка уже загружается — рендер не ждёт сеть.

### Меньше раздражителей (Part 4)
- Вместо блокирующего `alert()` на странице проекта — ненавязчивый тост.
- Этапы AI Director кликабельны: переход к любому пройденному пункту в один клик,
  а не серией «Назад».

### Проверено
- `npm run typecheck`, `npm run lint`, `npm run build` — чисто.
- Автомонтаж, montage-upgrades, AI Director (12 пунктов, chunks), offline-edit,
  production-plan, picture-lock, sound-design (143 проверки), motion-graphics,
  render-hardening, min-duration, music — все проходят.
- Реальный экспорт: `test:e2e`, `test:offline-render`, `test:motion-ffmpeg`, `test:vfx`,
  `test:normalize`, `test:colormatch` — рендер через @ffmpeg/core без ошибок.
