...
const regex = /function generateRuleBasedDecision\(request: AIAnalysisRequest\): AIEditDecision \{[\s\S]*?export async function transcribeAudio/m;

const newRuleBased = `async function generateRuleBasedDecision(request: AIAnalysisRequest): Promise<AIEditDecision> {
...
code = code.replace(regex, newRuleBased);
