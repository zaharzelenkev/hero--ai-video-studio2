...
const oldVisEnd = `    // Добавляем текст промпта на экран, если он есть
    if (_request.userPrompt && _request.userPrompt.length > 3 && script.scenes.length > 0) {
        script.scenes[0].captions.push({
            text: _request.userPrompt.slice(0, 60),
            offsetInScene: 0.2,
...
const newVisEnd = `    // Extract text requested by user
