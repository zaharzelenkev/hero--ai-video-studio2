  ...
  return (
    <div className="space-y-3">
      <section className="rounded-xl bg-gradient-to-r from-violet-900/30 to-fuchsia-900/30 border border-white/10 p-3 shadow-inner">
        <h3 className="text-xs font-bold text-amber-300 mb-2">Экспорт</h3>
        <div className="grid grid-cols-2 gap-2 mb-2">
          {(["mp4","webm","gif","mov"] as const).map(f => (
            <button key={f} onClick={() => setFormat(f)} className={`rounded-lg px-3 py-2 text-xs font-bold border transition ${format === f ? "bg-blue-600 text-white border-violet-400 shadow-lg" : "bg-white/5 text-slate-300 border-white/10 hover:bg-white/10"}`} aria-label={`Формат ${f}`}>{f.toUpperCase()}</button>
          ))}
        </div>
        <label className="text-[10px] text-slate-400 block mb-1">Качество (CRF / RGB)</label>
        <input type="range" min={0} max={100} value={quality} onChange={(e) => setQuality(parseInt(e.target.value))} className="w-full h-1.5 rounded-full bg-gradient-to-r from-slate-700 to-slate-600" aria-label="Качество" />
      ...
      <section className="rounded-xl bg-[#0d0d16] border border-white/10 p-3 shadow-inner">
        <h3 className="text-xs font-bold text-amber-300 mb-2">Разрешение и FPS</h3>
      ...
      <button
        ...
        disabled={exporting}
        className={`w-full rounded-xl py-3 text-sm font-bold shadow-lg transition ${exporting ? "bg-amber-600 text-white shadow-amber-500/30" : "bg-gradient-to-r from-blue-700 to-cyan-600 text-white shadow-blue-500/20 hover:brightness-110"}`}
