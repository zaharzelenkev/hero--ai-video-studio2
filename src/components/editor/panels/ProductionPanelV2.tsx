  ...
  return (
    <div className="space-y-3">
      <section className="rounded-xl bg-gradient-to-r from-violet-900/30 to-fuchsia-900/30 border border-white/10 p-3 shadow-inner">
        <h3 className="text-xs font-bold text-violet-300 mb-2">Настройки проекта</h3>
      ...
      <section className="rounded-xl bg-[#0d0d16] border border-white/10 p-3 shadow-inner">
        <h3 className="text-xs font-bold text-violet-300 mb-2">Стиль генерации</h3>
        <div className="text-xs text-slate-400">Параметры AI-стиля применяются при повторной генерации.</div>
        <button onClick={() => updateProject(p => ({ ...p, style: { ...p.style, pace: "medium" } }))} className="mt-2 rounded-lg bg-gradient-to-r from-violet-600 to-fuchsia-600 px-3 py-1.5 text-xs font-bold text-white shadow-lg">Обновить стиль</button>
      </section>
    </div>
  );
}
