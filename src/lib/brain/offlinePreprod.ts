...
export function buildOfflinePreprod(brief: DirectorBrief): PreProduction {
  ...
  p.planning = {
    ...
    directorNotes: [
      ...
      ...(brief.materials
        ? [`Материалы по брифингу: ${brief.materials}. Учти это при планировании съёмки и монтажа.`]
        : []),
      ...(brief.location
        ? [`Локация действия: ${brief.location}. Свет, звук и атмосферу сцен выстраиваем под неё.`]
        : []),
    ],
    teamTasks: [
  ...
  // --- LOCATIONS ---
  const locationName = brief.location?.trim() || "Основная локация героя";
  p.locations = [
    {
      id: uid("loc"),
      name: locationName,
      description: brief.location?.trim()
        ? `Пространство действия: ${brief.location.trim()}. Раскрывает атмосферу и контекст истории.`
        : "Пространство, где живёт/работает герой; раскрывает его характер.",
      mood: mood,
