...
/** Export current LUTs as .cube files (zip-like list) — simplified */
export async function exportLUTs(project: Project): Promise<{ name: string; content: string }[]> {
  const { cubeFileName } = await import("./editor/lut");
  ...
  // Fallback: just export known presets names — actual content generated in UI already
  const result: { name: string; content: string }[] = [];
  for (const p of presets) {
    result.push({ name: cubeFileName(p as any), content: `# MONTIQ LUT placeholder for ${p}\n# Full .cube generated by Color panel` });
  }
  return result;
}
